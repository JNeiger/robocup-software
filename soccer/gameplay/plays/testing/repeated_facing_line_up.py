...
class RepeatedFacingLineUp(play.Play):
    ...
    def __init__(self):
        ...
        for state in RepeatedFacingLineUp.State:
            self.add_state(state, behavior.Behavior.State.running)

        self.add_transition(behavior.Behavior.State.start,
                            RepeatedFacingLineUp.State.right, lambda: True,
                            'immediately')

        self.add_transition(
            RepeatedFacingLineUp.State.left,
            RepeatedFacingLineUp.State.pause,
            lambda: self.subbehavior_with_name('LineUp').state == behavior.Behavior.State.completed and time.time() - self.side_start > 1,
            'made it to left')

        self.add_transition(
            RepeatedFacingLineUp.State.right,
            RepeatedFacingLineUp.State.pause,
            lambda: self.subbehavior_with_name('LineUp').state == behavior.Behavior.State.completed and time.time() - self.side_start > 1,
            'made it to right')
        ...
        self.add_transition(
            RepeatedFacingLineUp.State.pause,
            RepeatedFacingLineUp.State.right,
            lambda: (time.time() - self.pause_start_time) > RepeatedFacingLineUp.Pause and self.prev_side == RepeatedFacingLineUp.State.left,
            'pause over')
        self.add_transition(
            RepeatedFacingLineUp.State.pause,
            RepeatedFacingLineUp.State.left,
            lambda: (time.time() - self.pause_start_time) > RepeatedFacingLineUp.Pause and self.prev_side == RepeatedFacingLineUp.State.right,
            'pause over')
    ...
    def on_exit_left(self):
        ...
        self.prev_side = RepeatedFacingLineUp.State.left
    ...
    def on_exit_right(self):
        self.remove_subbehavior('LineUp')
        self.prev_side = RepeatedFacingLineUp.State.right
    ...
    def execute_running(self):
        for lineup_bhvr in self.all_subbehaviors():
            for bhvr in lineup_bhvr.all_subbehaviors():
                if (bhvr.robot != None):
                    bhvr.robot.face(bhvr.robot.pos + robocup.Point(0, 100))
